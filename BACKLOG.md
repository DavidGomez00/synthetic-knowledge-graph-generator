...
## 'engine/'
- [ ] **`cycles.py`**: `break_cycles` is implemented and unit-tested offline (`tests/test_cycles.py`) but not yet run end to end against a graph store. Run `python -m skgg.cli.main -f french_royalty.json` and check the log shows the broken cycles, seed sizes and the follow-up completion. Seeds live only in the synthetic graph, so a `--skip-edb` rerun repeats the cycle-breaking; `apply_rule` in completion is not profile-capped, so a seeded cycle can overshoot its target frequency.
