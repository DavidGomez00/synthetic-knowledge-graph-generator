...
Rules are mined upstream (this repo doesn't mine them, e.g. with AMIE-style tools) and loaded from a CSV per dataset (`rules.rules_file` in each config), parsed by `core.rules.parse_rule_set`.
