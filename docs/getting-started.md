...
A walkthrough of setting up the project and running one experiment end-to-end, using the French Royalty dataset (`configurations/french_royalty_source.json`). `lung_cancer.json` also loads cleanly.
...
GraphDB doesn't auto-create repositories from a client connection the way Virtuoso auto-creates graphs — before running anything, open the GraphDB Workbench at `http://localhost:7200` and create a repository whose ID matches the config's `sparql_endpoint` (for `french_royalty_source.json`: `repositories/FrenchRoyalty` → repository ID `FrenchRoyalty`).
